...
-(void) performMaskingComputation:(ofxCvImage*)img {
...
-(void) performBackgroundSubtractionComputation {
    //Blur
...
-(void) performBlobTrackingComputation {
    pthread_mutex_lock(&mutex);
...
-(void) update:(NSDictionary *)drawingInformation{
    ...
    if(update && (bgSubtractionEnabled || contourFinderEnabled)){
...
-(void) drawInput:(NSRect)rect{
    ...
    if([[self.properties valueForKey:@"opticalFlowEnabled"] boolValue]){
...
-(void) startBackgroundThread:(id)param{
	while(1){
		...
		if(threadUpdateContour){
            [self performBlobTrackingBackgroundComputation];
			threadUpdateContour = false;
		}
        if(threadUpdateOpticalFlow){
            [self performOpticalFlowBackgroundComputation];           
